...
export const companiesAPI = {
  ...
  getAll: async () => {
    ...
    if (!contentType.includes("application/json")) {
      throw new Error(`Expected JSON from ${BASE}/companies but got "${contentType || "no content-type"}". ` + `Check that the client-api server is running and VITE_CLIENT_API_URL is set correctly.`);
    }
...
export const catalogAPI = {
  ...
  getAvailability: (serviceId, companyId, params = {}) => api.get(`/catalog/services/${serviceId}/availability`, { params: { company_id: companyId, ...params } }).then((r) => r.data),
};
