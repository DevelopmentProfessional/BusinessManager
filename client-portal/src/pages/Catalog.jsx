        ...
        <aside className="w-56 flex-shrink-0 space-y-6">
          <div className="card">
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={() => { setSearch(''); setCategory('all'); setMinPrice(''); setMaxPrice(''); setTab('all') }}
            ...
            <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm font-medium">
              {[['all','All'],['products','Products'],['services','Services']].map(([v, l]) => (
                <button key={v} onClick={() => setTab(v)}
                  className={`px-4 py-2 transition-colors ${tab === v ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                  {l}
                </button>
              ))}
            </div>
              ...
              {fs.length > 0 && (
                <section>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                    {fs.map(s => <ItemCard key={s.id} item={s} itemType="service" onSelect={handleSelect} />)}
                  </div>
                </section>
              )}
              {fp.length > 0 && (
                <section>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                    {fp.map(p => <ItemCard key={p.id} item={p} itemType="product" onSelect={handleSelect} />)}
                  </div>
                </section>
              )}
