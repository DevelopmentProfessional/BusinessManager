...
export default function OrderHistory() {
  ...
  const refreshStalePendingOrders = useCallback(
    async (loadedOrders) => {
  ...
  const handleRefreshStatuses = useCallback(
    async ({ showSuccessToast = true, showErrorToast = true } = {}) => {
    ...
    <Layout>
      <div className="p-3" style={{ paddingBottom: "5rem" }}>
        <div className="d-flex align-items-center justify-content-between mb-4">
          ...
          <div className="d-flex align-items-center gap-2">
            <button className="btn btn-outline-secondary btn-sm" onClick={handleRefreshStatuses} disabled={syncingStatuses}>
            ...
            <div className="btn-group btn-group-sm">
              {[
                ...
                <button key={v} className={`btn ${tab === v ? "btn-primary" : "btn-outline-secondary"}`} onClick={() => setTab(v)}>
                  {l}
                </button>
              ))}
            </div>
              ...
              <div>
                ...
                {orders.map((order) => (
                  <div key={order.id} className="card mb-2">
                    <div className="card-body d-flex align-items-center justify-content-between" style={{ cursor: "pointer" }} onClick={() => toggleExpand(order.id)}>
                      <div className="d-flex align-items-center gap-3">
                        <div>
                          <p className="fw-semibold small mb-0">Order #{order.id.split("-")[0].toUpperCase()}</p>
                          <p className="text-muted" style={{ fontSize: "0.75rem", marginBottom: 0 }}>
                            {format(new Date(order.created_at), "MMM d, yyyy · h:mm a")}
                          </p>
                        </div>
                        <span className={`badge ${STATUS_CLS[order.status] || "bg-secondary"} text-capitalize`}>{STATUS_LABELS[order.status] || order.status}</span>
                      </div>
                      <div className="d-flex align-items-center gap-3">
                        <span className="fw-bold">${order.total?.toFixed(2)}</span>
                        {expanded[order.id] ? <ChevronDownIcon style={{ width: 16, height: 16, color: "#9ca3af" }} /> : <ChevronRightIcon style={{ width: 16, height: 16, color: "#9ca3af" }} />}
                      </div>
                    </div>
                    {expanded[order.id] && <OrderItems order={order} clientName={client?.full_name} onPay={handlePayOrder} isPaying={payingOrderId === order.id} />}
                  </div>
                ))}
              </div>
            )}
                    ...
                    <div className="card-body d-flex align-items-center justify-content-between">
                      <div>
                        <div className="d-flex align-items-center gap-2 mb-1 flex-wrap">
                          <p className="fw-semibold small mb-0">{format(new Date(b.appointment_date), "EEEE, MMM d · h:mm a")}</p>
                          <span className={`badge ${STATUS_CLS[b.status] || "bg-secondary"}`}>{b.status}</span>
                          <span className={`badge ${b.booking_mode === "locked" ? "bg-success" : "bg-warning text-dark"}`}>{b.booking_mode === "locked" ? "Locked" : "Soft"}</span>
                        </div>
                        <p className="text-muted small mb-0">{b.duration_minutes} min</p>
                        {b.cancellation_charge > 0 && <p className="small text-warning mb-0">Cancellation fee: ${b.cancellation_charge.toFixed(2)}</p>}
                        {b.refund_amount > 0 && <p className="small text-success mb-0">Refunded: ${b.refund_amount.toFixed(2)}</p>}
                      </div>
                      {["pending", "confirmed"].includes(b.status) && (
                        <button className="btn btn-outline-danger btn-sm" onClick={() => handleCancelBooking(b.id)}>
                          Cancel
                        </button>
                      )}
                    </div>
            ...
            <div>
              ...
              {parseOptions(item.options_json).length > 0 && (
                <div className="text-muted">
                  Options:{" "}
                  {parseOptions(item.options_json)
                    .map((option) => option?.option_name || option?.name)
                    .filter(Boolean)
                    .join(", ")}
                </div>
