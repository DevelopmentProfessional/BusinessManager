  ...
  return (
    <Layout>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => navigate("/cart")} className="relative btn-secondary">
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          ...
          {filteredServices.length > 0 && (
            <section>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Services ({filteredServices.length})</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {filteredServices.map((s) => (
                  <ItemCard key={s.id} item={s} itemType="service" onSelect={handleItemSelect} />
                ))}
              </div>
            </section>
          )}
          ...
          {filteredProducts.length > 0 && (
            <section>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Products ({filteredProducts.length})</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {filteredProducts.map((p) => (
                  <ItemCard key={p.id} item={p} itemType="product" onSelect={handleItemSelect} />
                ))}
              </div>
            </section>
          )}
