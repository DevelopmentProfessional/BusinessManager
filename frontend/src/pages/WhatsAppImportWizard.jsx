      ...
      <div style={styles.card}>
        ...
        {step === 1 && (
          <form onSubmit={handleAuthSubmit}>
            <p style={styles.subtitle}>Enter your WhatsApp Business API credentials to import your catalog products.</p>

            <div style={styles.field}>
              <label style={styles.label}>Access Token *</label>
              <input type="password" style={styles.input} value={accessToken} onChange={(e) => setAccessToken(e.target.value)} placeholder="Your WhatsApp Business API access token" required />
            </div>

            <div style={styles.field}>
              <label style={styles.label}>Business Account ID *</label>
              <input type="text" style={styles.input} value={businessAccountId} onChange={(e) => setBusinessAccountId(e.target.value)} placeholder="Your business account ID" required />
            </div>
        ...
        {step === 2 && (
          <>
            <p style={styles.subtitle}>Found {products.length} products. Select which ones to import.</p>
            ...
            <button onClick={handleValidateFeatures} style={{ ...styles.button, ...(loading && styles.buttonDisabled) }} disabled={loading || selectedProducts.size === 0}>
              {loading ? "Validating..." : "Next: Validate Features"}
            </button>
            <button onClick={() => setStep(1)} style={{ ...styles.button, background: "#6b7280", marginLeft: 10 }}>
              Back
            </button>
          </>
        )}
        ...
        {step === 3 && (
          <>
            <p style={styles.subtitle}>Some product features are missing from your system. Add them before importing.</p>

            <div style={styles.featureList}>
              {missingFeatures.map((feature, i) => (
                <div key={i} style={styles.featureItem}>
                  <strong>{feature.feature_name}</strong>
                  <p style={{ fontSize: 12, color: "#666", marginTop: 5 }}>Missing options: {feature.options_to_add.join(", ")}</p>
                  <button
            ...
            <button onClick={() => setStep(4)} style={{ ...styles.button, ...(loading && styles.buttonDisabled) }} disabled={loading || missingFeatures.some((f) => !newFeatures[f.feature_name])}>
              {loading ? "Processing..." : "Continue to Import"}
            </button>
            <button onClick={() => setStep(2)} style={{ ...styles.button, background: "#6b7280", marginLeft: 10 }}>
              Back
            </button>
          </>
        )}
          ...
          <>
            ...
            {importResult ? (
              <div style={styles.success}>
                ...
                <p>
                  Imported: {importResult.imported}, Skipped (duplicates): {importResult.skipped}
                </p>
              </div>
            ) : (
              <>
                <button onClick={handleImport} style={{ ...styles.button, background: "#10b981", ...(loading && styles.buttonDisabled) }} disabled={loading}>
            ...
            {importResult && (
              <button onClick={() => navigate("/inventory")} style={{ ...styles.button, background: "#4f46e5", marginTop: 15, width: "100%" }}>
                Go to Inventory
              </button>
            )}
          </>
