...
const mapDbItem = (d) => ({
  ...
  price: d.unit_price,
  quantity: d.quantity,
  selectedOptions: d.options_json
    ? (() => {
        try {
          return JSON.parse(d.options_json);
        } catch {
          return [];
        }
      })()
...
export default function Modal_Client_Cart({ isOpen, onClose, client }) {
  ...
  const updateQty = (cartKey, delta) => {
    setCartItems((prev) => {
      const next = prev.map((item) => (item.cartKey === cartKey ? { ...item, quantity: item.quantity + delta } : item)).filter((item) => item.quantity > 0);
                  ...
                  <div key={item.cartKey} className="border-bottom border-gray-100 dark:border-gray-700">
                    ...
                    <div className="d-flex align-items-center gap-2 py-2 px-3">
                      ...
                      <div className="d-flex align-items-center gap-1 flex-shrink-0">
                        {/* Qty controls */}
                        <button type="button" onClick={() => updateQty(item.cartKey, -1)} className="btn btn-outline-secondary btn-sm rounded-circle d-flex align-items-center justify-content-center" style={{ width: 28, height: 28, padding: 0 }}>
