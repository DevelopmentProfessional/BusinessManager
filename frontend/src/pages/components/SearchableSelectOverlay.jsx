...
function isSearchableSelect(select) {
  return select instanceof HTMLSelectElement
    ...
    && !select.multiple
    && select.size <= 1
    && select.dataset.searchableSelectDisabled !== 'true'
    && !select.closest('[data-searchable-select-disabled="true"]');
}
...
function getPanelStyle(select) {
  ...
  const width = Math.min(
    ...
    window.innerWidth - (PANEL_SIDE_MARGIN * 2),
  );
  const left = Math.min(
    Math.max(rect.left, PANEL_SIDE_MARGIN),
    window.innerWidth - width - PANEL_SIDE_MARGIN,
  );
  ...
  return createPortal(
    <div className="searchable-select-overlay-root">
      <button
        ...
        aria-label="Close searchable dropdown"
      ...
      >
        <div className="searchable-select-panel-header">
          ...
          {selectedOption && !searchTerm && (
            <div className="searchable-select-current-value">Selected: {selectedOption.label || 'Blank value'}</div>
          )}
        </div>
