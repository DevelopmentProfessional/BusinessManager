...
function AccordionSection({ label, count, isOpen, onToggle, onClear, children, helpText, helpKey, showHelp, setShowHelp, helpPos, setHelpPos }) {
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-700 rounded">
      <div className="d-flex align-items-center justify-content-between px-3 py-2" onClick={onToggle} style={{ cursor: "pointer", userSelect: "none" }}>
      ...
      {isOpen && <div className="px-3 pb-2 pt-1 d-flex flex-column gap-1 border-top border-gray-100 dark:border-gray-700">{children}</div>}
    </div>
  );
}

// ─── 3 STATE & EFFECTS ───────────────────────────────────────────────────
export default function Modal_Filter_Schedule({ isOpen, onClose, employees, clients, services, filters, onApply, onClear, approvedLeaves }) {
