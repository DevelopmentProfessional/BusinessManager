 ...
 */
export default function Image_Square({ src, alt = "", fallbackContent = null, className = "", style = {}, onError, size = "full" }) {
  const sizeClasses = {
    sm: "w-20 h-20", // 80px
    md: "w-30 h-30", // 120px
    lg: "w-50 h-50", // 200px
  ...
  const containerClass = size === "full" ? "square-image-container" : `${sizeClasses[size] || sizeClasses.full} relative overflow-hidden rounded-lg`;
  ...
  return (
    <div className={`${containerClass} ${className}`} style={style}>
      {src ? <img src={src} alt={alt} className={size === "full" ? "square-image" : "w-full h-full object-cover object-center"} onError={handleImageError} /> : null}

      {/* Fallback content - always present but conditionally visible */}
      <div className={`${size === "full" ? "square-image-placeholder" : "absolute inset-0 flex items-center justify-center"} ${src ? "hidden" : "flex"}`} style={{ display: src ? "none" : "flex" }}>
        {fallbackContent || <div className="text-gray-400 text-xs text-center">No Image</div>}
      </div>
    </div>
  );
}
