...
function getBiweekRange(offset) {
  ...
  return {
    start: iso(biStart), end: iso(biEnd),
...
export default function Modal_Wages({ employees = [], onClose }) {
  ...
  const [payFreqView, setPayFreqView] = useState(() => {
    ...
    employees.forEach(e => { const f = (e.pay_frequency || '').toLowerCase(); if (f in cnt) cnt[f]++; });
  ...
  useEffect(() => {
    ...
    payrollAPI.getAll()
      .then(r => { if (!cancelled) { const d = r?.data ?? r ?? []; setAllSlips(Array.isArray(d) ? d : []); } })
  ...
  const handlePaySingle = async (emp) => {
    ...
    try {
      ...
      if (isH) { payload.hours_worked = parseFloat(form.hours_worked || 0); payload.hourly_rate_snapshot = emp.hourly_rate; }
      else      { payload.gross_amount = parseFloat(form.gross_amount || 0); }
      const res = await payrollAPI.processPayment(emp.id, payload);
      const slip = res?.data ?? res;
      if (slip?.id) { setAllSlips(p => [...p, slip]); setExpandedId(null); }
    } catch (err) {
  ...
  const handlePayAll = async () => {
    ...
    for (const emp of unpaid) {
      const amt = calcAmount(emp, period.start, period.end);
      const isH = (emp.employment_type || '').toLowerCase() === 'hourly';
      if (!amt) { setPayAllProgress(p => ({ ...p, done: p.done + 1, errors: p.errors + 1 })); continue; }
      try {
        const payload = {
          pay_period_start: `${period.start}T00:00:00`,
          pay_period_end:   `${period.end}T23:59:59`,
        ...
        if (isH) { payload.hours_worked = 40; payload.hourly_rate_snapshot = emp.hourly_rate; }
        else      { payload.gross_amount = parseFloat(amt.toFixed(2)); }
        const res = await payrollAPI.processPayment(emp.id, payload);
        const slip = res?.data ?? res;
        if (slip?.id) { newSlips.push(slip); setPayAllProgress(p => ({ ...p, done: p.done + 1, paid: p.paid + 1 })); }
        else setPayAllProgress(p => ({ ...p, done: p.done + 1, errors: p.errors + 1 }));
      } catch { setPayAllProgress(p => ({ ...p, done: p.done + 1, errors: p.errors + 1 })); }
    }
  ...
  const handleOtSubmit = async () => {
    ...
    setOtLoading(true); setOtResult(null);
    try {
        ...
        <div className="d-flex gap-1" style={{ marginBottom: -1 }}>
          {[
            ...
            <button
              ...
              className={`btn btn-sm px-3 ${activeTab === tab.key ? 'btn-primary' : 'btn-outline-secondary'}`}
      ...
      <div className="flex-grow-1 overflow-auto no-scrollbar">
        ...
        {activeTab === 'payroll' && (
          <>
            {/* Sticky controls */}
            <div
              className="bg-white dark:bg-gray-800 border-bottom border-gray-100 dark:border-gray-700 px-3 py-2"
            ...
            >
              ...
              <div className="d-flex gap-1 mb-2">
                {['weekly', 'biweekly', 'monthly'].map(f => (
                  <button
                    key={f}
                    type="button"
                    onClick={() => { setPayFreqView(f); setPeriodOffset(0); setExpandedId(null); setPayAllProgress(null); }}
              ...
              <div className="d-flex align-items-center gap-2 mb-2">
                <button
                  type="button"
                  onClick={() => { setPeriodOffset(p => p - 1); setExpandedId(null); setPayAllProgress(null); }}
                ...
                <div className="flex-grow-1 text-center">
                  <div className="fw-semibold" style={{ fontSize: '0.85rem' }}>{period.label}</div>
                  <div className="text-muted" style={{ fontSize: '0.68rem' }}>{period.start} → {period.end}</div>
                </div>
                <button
                  type="button"
                  onClick={() => { setPeriodOffset(p => p + 1); setExpandedId(null); setPayAllProgress(null); }}
            ...
            {slipsLoading ? (
              ...
              <div className="text-center text-muted py-5 small">
                ...
                <div style={{ fontSize: '0.72rem' }}>Set <strong>Pay Frequency</strong> on employee profiles to see them here.</div>
              </div>
            ) : visibleEmps.map(emp => {
                ...
                <div key={emp.id} className={`border-bottom border-gray-100 dark:border-gray-700 ${paidNow ? 'bg-success-subtle' : ''}`}>
                  ...
                  <div className="d-flex align-items-center gap-2 px-3 py-2">
                    ...
                    <div
                      ...
                      style={{ width: 36, height: 36, fontSize: '0.7rem', background: paidNow ? '#16a34a' : '#9ca3af' }}
                    >
                    ...
                    <div className="flex-grow-1 min-w-0">
                      ...
                      <div className="d-flex align-items-center gap-1 flex-wrap" style={{ fontSize: '0.7rem' }}>
                        <span className={`badge ${roleBadge(emp.role)}`} style={{ fontSize: '0.6rem' }}>{emp.role}</span>
                        <span className="text-muted">
                          {isH
                            ? `$${emp.hourly_rate}/hr · hourly`
                            : emp.salary
                            ? `$${Number(emp.salary).toLocaleString()}/yr · salary`
                            : 'No rate set'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="px-3 pb-3 pt-1" style={{ background: 'var(--bs-tertiary-bg, #f8f9fa)' }}>
                      ...
                      <div className="row g-2">
                        <div className="col-6">
                          ...
                          {isH && emp.hourly_rate && (
                            <div className="text-muted mt-1" style={{ fontSize: '0.68rem' }}>@ ${emp.hourly_rate}/hr</div>
                          )}
                        </div>
                        <div className="col-6">
        ...
        {activeTab === 'onetime' && (
          <div className="p-3">
            <p className="small text-muted mb-3">
              Issue a bonus, commission, reimbursement, or any other one-off payment to an employee.
              These appear in the employee's pay history and do not interfere with regular payroll cycles.
            </p>
            ...
            {otResult?.ok === false && (
              <div className="alert alert-danger py-2 px-3 small mb-3">{otResult.error}</div>
            )}
              ...
              <div className="col-6">
                ...
                <div className="input-group input-group-sm">
                  ...
                  <input
                    ...
                    value={ot.amount}
                    onChange={e => setOt(p => ({ ...p, amount: e.target.value }))}
                  />
                </div>
              </div>
              <div className="col-6">
                ...
                <input
                  ...
                  value={ot.date}
                  onChange={e => setOt(p => ({ ...p, date: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="mb-3">
              ...
              <input
                type="text" className="form-control form-control-sm"
                placeholder="e.g. Year-end bonus, Q1 commission…"
                value={ot.notes}
                onChange={e => setOt(p => ({ ...p, notes: e.target.value }))}
              />
            </div>
              ...
              filteredHistory.map(slip => {
                ...
                return (
                  <div key={slip.id} className="d-flex align-items-center gap-2 px-3 py-2 border-bottom border-gray-100 dark:border-gray-700">
                    <div
                      className="flex-shrink-0 rounded-circle d-flex align-items-center justify-content-center fw-bold"
                      style={{ width: 32, height: 32, fontSize: '0.65rem', background: '#d1fae5', color: '#065f46' }}
                    >
                      $
                    </div>
                    <div className="flex-grow-1 min-w-0">
      ...
      <div className="flex-shrink-0 pt-2 pb-4 px-3 border-top border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
        <div className="d-flex align-items-center justify-content-center gap-3">
          ...
          {activeTab === 'payroll' && unpaidCount > 0 && !slipsLoading && (
            <Button_Toolbar
              icon={BanknotesIcon}
              label={`Pay All ${unpaidCount}`}
              onClick={handlePayAll}
              disabled={!!payAllBusy}
              className="btn-app-primary"
              badge={payAllBusy
                ? <span className="spinner-border ms-1" style={{ width: 11, height: 11, borderWidth: 2 }} />
                : null}
            />
          )}
        </div>
