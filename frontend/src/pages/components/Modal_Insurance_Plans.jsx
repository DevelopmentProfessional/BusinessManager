            ...
            <div className="d-flex flex-column gap-2 pb-2">
              {insurancePlans.map((plan) => (
                <div key={plan.id} className={`d-flex align-items-center justify-content-between p-2 border rounded ${!plan.is_active ? "opacity-60" : ""}`}>
                  <button type="button" className="btn btn-sm btn-outline-danger d-flex align-items-center justify-content-center me-2" style={{ width: "3rem", height: "3rem" }} onClick={() => onDelete(plan.id)} title="Delete">
                  ...
                  <div className="d-flex gap-1">
                    ...
                    <button type="button" className="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center" style={{ width: "3rem", height: "3rem" }} onClick={() => setEditingPlan({ ...plan })} title="Edit">
        ...
        <div className="flex-shrink-0 border-top border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 pt-2 pb-4">
          <form onSubmit={onSave} className="d-flex flex-column gap-2">
            ...
            <div className="d-flex align-items-center">
              <div style={{ width: 40 }}>
                {editingPlan && (
                  <button type="button" onClick={() => setEditingPlan(null)} className="btn btn-outline-secondary btn-sm p-1 d-flex align-items-center justify-content-center" style={{ width: "3rem", height: "3rem" }} title="Cancel edit">
                    <XMarkIcon style={{ width: 18, height: 18 }} />
                  </button>
                )}
                {!editingPlan && (
                  <button type="button" onClick={() => onClose()} className="btn btn-outline-secondary btn-sm p-1 d-flex align-items-center justify-content-center" style={{ width: "3rem", height: "3rem" }} title="Close modal">
                    <XMarkIcon style={{ width: 18, height: 18 }} />
                  </button>
                )}
              </div>
              <div className="flex-grow-1 d-flex justify-content-center">
                <button type="submit" className="btn btn-primary btn-sm p-1 d-flex align-items-center justify-content-center" style={{ width: "3rem", height: "3rem" }} title={editingPlan ? "Save Changes" : "Add Plan"}>
