      ...
      <div style={{ background: "var(--bs-body-bg)", borderRadius: 12, width: "100%", maxWidth: 900, maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 24px 64px rgba(0,0,0,0.35)" }}>
        ...
        <div style={{ flex: 1, overflow: "hidden", display: "flex", minHeight: 0 }}>
          ...
          <div style={{ width: 220, borderRight: "1px solid var(--bs-border-color)", overflowY: "auto", padding: "10px 0" }}>
            ...
            {!loading && rules.length === 0 && <div style={{ padding: "8px 12px", fontSize: "0.78rem", color: "#9ca3af" }}>No rules yet.</div>}
            {rules.map((rule) => (
              <div
                key={rule.id}
                ...
                style={{
                  padding: "7px 12px",
                  cursor: "pointer",
                  borderLeft: editingId === rule.id ? "3px solid #6366f1" : "3px solid transparent",
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ padding: "10px 14px 6px", borderBottom: "1px solid var(--bs-border-color)" }}>
              ...
              {form.applies_to === "selected" && <input type="text" placeholder="Search products…" value={productSearch} onChange={(e) => setProductSearch(e.target.value)} style={{ ...inp, fontSize: "0.78rem" }} />}
            </div>

            {/* Product list */}
            <div style={{ flex: 1, overflowY: "auto", padding: "6px 14px" }}>
              {form.applies_to === "all" ? (
                <div style={{ padding: "24px 0", textAlign: "center", color: "#9ca3af", fontSize: "0.85rem" }}>This rule will apply to all sellable inventory items.</div>
              ) : (
        ...
        <div style={{ borderTop: "2px solid var(--bs-border-color)", padding: "12px 18px", background: "var(--bs-tertiary-bg, #f9fafb)" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-end", marginBottom: 10 }}>
            ...
            <div style={{ flex: "0 1 90px" }}>
              ...
              <input type="number" min="0" step="0.01" style={inp} value={form.discount_value} onChange={(e) => setForm((p) => ({ ...p, discount_value: e.target.value }))} placeholder={form.discount_type === "percentage" ? "15" : "5.00"} />
            </div>
            <div style={{ flex: "0 0 auto", display: "flex", alignItems: "center", gap: 6, paddingBottom: 4 }}>
              ...
              <label htmlFor="dr-active" style={{ fontSize: "0.8rem", cursor: "pointer" }}>
                Active
              </label>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-end", marginBottom: 10 }}>
            ...
            <div style={{ flex: "0 0 auto", display: "flex", alignItems: "center", gap: 6, paddingBottom: 4 }}>
              ...
              <label htmlFor="dr-recur" style={{ fontSize: "0.8rem", cursor: "pointer" }}>
                Recurring
              </label>
            </div>
          </div>
              ...
              {form.recur_frequency === "weekly" && (
                <div>
                  ...
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                    {DAYS.map((d) => (
                      <button key={d} onClick={() => toggleDay(d)} style={chip(form.recur_days.includes(d))}>
                        {d}
                      </button>
                    ))}
                  </div>
