...
#   2026-09-11 | GitHub Copilot | Added standard company-branded payslip template
# ============================================================
...
_STANDARD_TEMPLATES = [
    ...
    },
    {
        ...
        "description": "Standard company-branded employee pay slip.",
        "content": (
            '<div style="font-family:Arial,sans-serif;color:#111;max-width:720px;margin:0 auto;">'
            ...
            '<div><div style="max-width:180px;margin-bottom:8px;">{{company.logo}}</div><h2 style="margin:0;">{{company.name}}</h2>'
            '<div style="font-size:12px;color:#555;">{{company.address}}<br>{{company.email}} | {{company.phone}}</div></div>'
            '<div style="text-align:right;"><h1 style="margin:0 0 8px;font-size:26px;">PAY SLIP</h1>'
            '<div><strong>Pay Slip #:</strong> {{payslip.number}}</div><div><strong>Paid:</strong> {{payslip.paid_at}}</div></div></div>'
            '<table style="width:100%;margin-bottom:20px;"><tr><td><strong>Employee</strong><br>{{employee.first_name}} {{employee.last_name}}</td>'
            '<td><strong>Role</strong><br>{{employee.role}}</td><td><strong>Pay Period</strong><br>{{payslip.period_start}} - {{payslip.period_end}}</td></tr></table>'
            '<table style="width:100%;border-collapse:collapse;margin-bottom:20px;">'
            '<tr style="background:#f3f4f6;"><th style="padding:8px;text-align:left;">Description</th><th style="padding:8px;text-align:right;">Amount</th></tr>'
            '<tr><td style="padding:8px;border-bottom:1px solid #ddd;">Gross Pay</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right;">{{payslip.gross}}</td></tr>'
            '<tr><td style="padding:8px;border-bottom:1px solid #ddd;">Insurance Deduction</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right;">{{payslip.insurance_deduction}}</td></tr>'
            '<tr><td style="padding:8px;border-bottom:1px solid #ddd;">Other Deductions</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right;">{{payslip.other_deductions}}</td></tr>'
            '<tr style="font-weight:700;font-size:16px;"><td style="padding:10px 8px;">Net Pay</td><td style="padding:10px 8px;text-align:right;">{{payslip.net}}</td></tr></table>'
            '<div style="font-size:12px;color:#555;"><strong>Payment details</strong><br>Service Revenue: {{payslip.service_revenue}} | Base Pay: {{payslip.base_pay}} | Compensation: {{payslip.compensation_percentage}}</div>'
            '<div style="margin-top:28px;padding-top:12px;border-top:1px solid #ddd;font-size:11px;color:#777;">Generated by {{company.name}} on {{date}} at {{time}}.</div>'
            '</div>'
        ),
    },
    {
    ...
    stmt = select(DocumentTemplate).where(
        ...
        or_(
            and_(
                ...
                or_(DocumentTemplate.company_id == None, DocumentTemplate.company_id == ""),
            ),
